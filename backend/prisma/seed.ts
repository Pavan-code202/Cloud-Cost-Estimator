import { PrismaClient, pricing_pricing_model, pricing_category } from '@prisma/client';
...
async function main() {
  for (const item of records) {
    const provider = await prisma.cloudProvider.upsert({
      where: { code: item.provider },
      update: { name: item.providerName },
      create: { code: item.provider, name: item.providerName }
    });

    const service = await prisma.service.upsert({
      where: { providerId_name: { providerId: provider.id, name: item.service } },
      update: {},
      create: { providerId: provider.id, name: item.service, type: 'COMPUTE' }
    });

    const region = await prisma.region.upsert({
      where: { providerId_code: { providerId: provider.id, code: item.regionCode } },
      update: { name: item.region },
      create: { providerId: provider.id, name: item.region, code: item.regionCode }
    });

    const configuration = await prisma.configuration.upsert({
      where: {
        serviceId_regionId_name: {
    ...
    await prisma.pricing.upsert({
      where: {
        configurationId_pricingModel_category_effectiveDate: {
          configurationId: configuration.id,
          pricingModel: pricing_pricing_model.ON_DEMAND,
          category: pricing_category.COMPUTE_HOURLY,
          effectiveDate: new Date(`${item.effective}T00:00:00.000Z`)
        }
      },
      update: {
        price: item.price,
        source: 'OFFICIAL_PROVIDER_DATA'
      },
      create: {
        configurationId: configuration.id,
        pricingModel: pricing_pricing_model.ON_DEMAND,
  ...
  console.log('✓ Database seeded with compute pricing');
}
