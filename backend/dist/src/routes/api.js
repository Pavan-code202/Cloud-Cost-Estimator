...
}).refine(data => data.minimumWorkloadPercent <= data.averageWorkloadPercent, { message: 'Minimum workload must be <= average workload' }).refine(data => data.averageWorkloadPercent <= data.peakWorkloadPercent, { message: 'Average workload must be <= peak workload' }).refine(data => data.peakHoursPerDay <= data.usageHoursPerDay, { message: 'Peak hours per day must be <= usage hours per day' });
