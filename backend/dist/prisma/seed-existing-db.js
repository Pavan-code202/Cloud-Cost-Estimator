...
async function main() {
    ...
    await prisma.$transaction(async (tx) => {
        for (const r of records) {
            ...
            const providers = await tx.$queryRaw `SELECT provider_id FROM cloud_providers WHERE provider_code = ${r.provider} LIMIT 1`;
            ...
            const [service] = await tx.$queryRaw `SELECT service_id FROM services WHERE provider_id = ${providerId} AND service_name = ${r.service} LIMIT 1`;
            const [region] = await tx.$queryRaw `SELECT region_id FROM regions WHERE provider_id = ${providerId} AND region_code = ${r.regionCode} LIMIT 1`;
            ...
            const [configuration] = await tx.$queryRaw `SELECT configuration_id FROM configurations WHERE service_id = ${service.service_id} AND region_id = ${region.region_id} AND configuration_name = ${r.configuration} ORDER BY configuration_id LIMIT 1`;
    ...
    const counts = await prisma.$queryRaw `SELECT (SELECT COUNT(*) FROM cloud_providers) providers, (SELECT COUNT(*) FROM services) services, (SELECT COUNT(*) FROM regions) regions, (SELECT COUNT(*) FROM configurations) configurations, (SELECT COUNT(*) FROM pricing) pricing`;
